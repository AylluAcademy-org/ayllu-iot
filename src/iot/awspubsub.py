...
def on_connection_resumed(connection, return_code, session_present, **kwargs):
    ...
    if return_code == mqtt.ConnectReturnCode.ACCEPTED and not session_present:
        ...
        # Cannot synchronously wait for resubscribe result because we're on the
...
def on_message_received(topic, payload):
    ...
    try:
        if 'client_id' not in messages:
            # Build array depending on the number of messages to be received
            # (Number is set by the seq identifier in the json file)
            obj.append(messages)
            nowTimeStamp = datetime.now()
            print("Received message from topic '{}' : '{}' : {}".format(
                nowTimeStamp, topic, obj))
            # # Waits until the object lenght is equal to the sequence number
            # in the message
        ...
        if obj:
            print("Message without seq param, no command executed {}"
                  .format(obj.pop(0)))
        else:
